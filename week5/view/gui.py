...
class Gui(tk.Tk):
    ...
    def render(self):
        ...
        for row_index in range(self.__environment.get_height()):
            for col_index in range(self.__environment.get_width()):
                ...
                # Set the current agent's colour based on its type
                agent_color = (
                    self.__agent_colours.get(agent.__class__) if agent else "white"
                )
    ...
    def __init_world(self):
        ...
        self.render()  # Draw the initial grid
    ...
    def update_legend(self):
        ...
        for agent_class, count in sorted_counts:
            color_label = tk.Label(
                self.legend_panel, bg=self.__agent_colours[agent_class], width=2, height=1
            )
